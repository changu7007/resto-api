...
const assignTablesForWaiters = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!staffId || !Array.isArray(assignedTables)) {
        throw new bad_request_1.BadRequestsException("Invalid staff assignment format. Expecting { staffId: string, assignedTables: string[] }", root_1.ErrorCode.UNPROCESSABLE_ENTITY);
    }
    yield __1.prismaDB.$transaction((tx) => __awaiter(void 0, void 0, void 0, function* () {
        // Verify staff exists and is a waiter
        const staff = yield tx.staff.findFirst({
            where: {
                id: staffId,
                restaurantId: getOutlet.id,
            },
        ...
        if (staff.role !== "WAITER") {
            throw new bad_request_1.BadRequestsException(`Staff ${staff.name} is not a WAITER`, root_1.ErrorCode.UNPROCESSABLE_ENTITY);
        }
        // Verify all tables exist
        if (assignedTables.length > 0) {
            const tables = yield tx.table.findMany({
                where: {
        ...
        const otherWaiters = yield tx.staff.findMany({
            where: {
                restaurantId: getOutlet.id,
                role: "WAITER",
                id: { not: staffId },
        ...
        for (const waiter of otherWaiters) {
            const newAssignedTables = waiter.assignedTables.filter((tableId) => !assignedTables.includes(tableId));
